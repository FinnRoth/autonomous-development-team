...
9. **board-api is the authoritative structured ticket store and the ONLY location for ticket data.** No markdown ticket files (`docs/tickets/*.md`) exist. No `board.md` exists. Agents read and write tickets exclusively via board-api MCP tools.
...
6. Only then edit.
7. Call `board_list_tickets()` to check board state.
