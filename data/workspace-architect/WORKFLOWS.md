...
- **Exit:** all dependent artifacts updated; FE/BE notified.
- **Actions:**
  1. If the ADR mandates an OpenAPI change: edit `docs/architecture/api/openapi.yaml`, run skill `validate-openapi`, run skill `generate-contracts`, commit on a new branch, open PR.
  ...
  4. Send `handoff` to `backend` and `frontend` summarizing what changed and pointing to the merged ADR + contracts paths.
...
- **Actions:**
  ...
  5. Write `docs/architecture/audit-<YYYY-MM-DD>.md` summarizing.
