...
- **Actions:**
  ...
  4. Move processed message to `inbox/processed/<original-filename>`.
...
- **Actions:**
  1. Push branch.
  2. Open PR against `<project>-docs` main branch. Title `[<TICKET-ID>] ADR-NNN: <title>`. Body lists Alternatives and key Consequences.
  3. Send `handoff` to `reviewer` with `artifact_paths: ["docs/architecture/adr/ADR-NNN-<slug>.md"]`.
  ...
  5. Wait for reviewer verdict (poll `inbox/`).
...
- **Actions:**
  ...
  4. After merge, `git -C docs pull --ff-only`.
...
- **Actions:**
  1. If the ADR mandates an OpenAPI change: edit `docs/architecture/api/openapi.yaml`, run skill `validate-openapi`, run skill `generate-contracts`, commit on a new branch, open PR. **Do NOT send FE/BE handoffs until the contracts PR is merged and contracts are on `main`.**
  2. If the ADR mandates a data-model change: run skill `propose-data-model-change`, regenerate Mermaid ER, commit, open PR.
  3. If the ADR mandates a folder layout change: edit `folder-structure.md`, commit `.gitkeep` skeleton files to `project/` on a new branch, open PR against `<project>`.
  4. After all PRs are merged: send `handoff` to `backend` AND `frontend` summarizing what changed, pointing to the merged ADR + contracts paths, and explicitly stating: "Generated contracts at `project/.architecture/contracts/` are ready. Pull `main` before starting."
  5. **Compatibility note in handoffs:** every PROPAGATE handoff must include a section `## Compatibility` listing: which API operations changed (added/removed/modified), which types changed, and which existing callers (if any) need updates. Backend and frontend must acknowledge receipt before starting work on affected tickets.
...
- **Actions:**
  ...
  5. **Frontend–backend compatibility check:** run `generate-contracts` against the current `openapi.yaml`. Compare the generated types against what both `project/backend/` and `project/frontend/` import from `project/.architecture/contracts/`. Any mismatch = contract drift.
  6. Write `docs/architecture/audit-<YYYY-MM-DD>.md` summarizing all findings including compatibility status.
- **Output artifacts:** `docs/architecture/audit-<YYYY-MM-DD>.md`.
...
- If I am uncertain which state I am in on wake, I scan `outbox/` for the last 24h and `docs/architecture/adr/` for any `proposed` ADR, then resume.
- **Contracts always precede feature work.** Generated contracts in `project/.architecture/contracts/` must be on `main` BEFORE either backend or frontend claims a ticket that depends on them. If a developer claims a ticket and contracts are not yet available, I file a `question` to that developer flagging the dependency.
