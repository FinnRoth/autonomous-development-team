...
2. `git pull` `project/` to get the fix.
...
9. If this was the last open S1/S2 bug for the Story AND all cases pass → send a final "Story qa-complete" handoff to project-lead with the coverage summary.
...
2. Pick up other Stories from the qa column if any (parallel work is fine; mark which Story is blocked on what in the coverage matrix).
