...
> **Note (board-api model):** As of the board-api integration, project-lead no longer sends explicit assignment handoffs to backend for routine task dispatch. Backend self-polls `board_get_ready_tickets(owner="backend")` each heartbeat and self-claims via `board_claim_ticket`. The handoff protocol from project-lead remains for: (a) priority overrides/re-assignments requiring context, (b) escalation responses, (c) out-of-band blocker notifications.
