...
outputs: board-api ticket marked in_progress; docs mirror commit; local branch frontend/<TICKET-ID>-<slug>; memory note.
---
...
```
On non-fast-forward: log to memory; the board-api claim already succeeded, proceed anyway.
Edit `docs/tickets/<TICKET-ID>.md`: set frontmatter `status: in_progress`.
```sh
cd docs
git add tickets/<TICKET-ID>.md
git commit -m "[<TICKET-ID>] frontend claims ticket"
git push
```
On push failure: log to memory and continue.
