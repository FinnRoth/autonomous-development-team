...
```
IDLE → CLAIM → INTAKE → PAGE_INVENTORY → FLOWS → WIREFRAMES → COMPONENT_PASS
...
- **Entry condition:** HEARTBEAT or IDLE detected a ready ticket via `board_get_ready_tickets(owner="uiux")`.
- **Exit condition:** `board_claim_ticket` returned 200 (ticket is mine); branch `uiux/<TICKET-ID>-<slug>` will be created in INTAKE.
- **Actions:**
  1. Call `board_claim_ticket(ticket_id=<id>, agent="uiux")`.
  2. On 200: call `board_transition_ticket(ticket_id=<id>, status="in_progress")`. Proceed to INTAKE.
  3. On 409 (race): re-poll `board_get_ready_tickets` once. If another ticket is available, claim it. If none, return to IDLE.
  4. Log claim in `memory/YYYY-MM-DD.md`.
- **Output artifacts:** board ticket in `in_progress`; memory entry.
- **On-error:** any non-200/409 response → log error in memory, return to IDLE.
