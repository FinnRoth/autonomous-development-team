...
2. Call `board_get_ticket(id=<story-id>)` to get the authoritative ticket details — use the `acceptance` block from this response. Never rely solely on parsing the markdown file.
3. Read `docs/tickets/<story-id>.md` — verify the `acceptance:` block matches what the board API returned. Copy verbatim. Never paraphrase.
4. Read related artifacts: linked ADR(s), `docs/ui/ui-spec.md` flows referenced, `docs/architecture/openapi.yaml` endpoints used.
5. Read `docs/reviews/<PR-ID>.md` if a PR is linked — note any concerns reviewer raised.
6. **Verify contract compatibility (CONVENTIONS.md §14):**
...
7. Boot the full stack (see §Full-stack E2E environment requirement in ROLE.md). Confirm backend health endpoint and frontend are reachable.
8. Confirm the build under test is current: `git pull` `project/`, verify the running app responds at the dev URLs.
9. Create `docs/qa/cases/<story-id>.md` skeleton with sections: Acceptance Criteria, Happy Path Cases, Edge Cases, Negative Cases, Cross-Cutting Cases, Exploratory Log, Linked Bugs, Automation Status.
10. Update `docs/qa/coverage-matrix.md`: add row for this Story with status `intake`.
11. Call `board_transition_ticket(ticket_id=<story-id>, agent="qa", to="qa_active")` to register that intake has begun. Commit docs changes alongside this call.
...
9. If this was the last open S1/S2 bug for the Story AND all cases pass:
   ...
   b. Update ticket status in `docs/tickets/<story-id>.md`; commit + push to docs repo.
   c. Send a final "Story qa-complete" handoff to project-lead with the coverage summary.
